  ...
  return (
    <div className="min-h-screen bg-background text-foreground antialiased selection:bg-primary/30 selection:text-primary-foreground">
      ...
      <main className="relative overflow-hidden">
        {/* ─── HERO ─── */}
        <section id="hero" className="hero-gradient pt-20 md:pt-28 pb-12 md:pb-16 px-2 md:px-10 flex flex-col items-center">
          <div className="max-w-6xl w-full text-center">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-border bg-card mb-6 md:mb-8">
              <span className="flex h-2 w-2 rounded-full bg-primary animate-pulse"></span>
              <span className="text-xs uppercase tracking-widest text-primary font-medium">Now supporting 7 Currencies</span>
            </div>
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-headline font-bold mb-4 md:mb-6 max-w-4xl mx-auto leading-[1.1] tracking-tight">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-4 mb-12 md:mb-16">
              <button
                onClick={handleComplete}
                disabled={loading}
                className="bg-primary text-primary-foreground font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-xl text-sm sm:text-lg hover:brightness-110 transition-all emerald-drop disabled:opacity-50 w-full sm:w-auto"
              >
                {loading ? 'Setting up...' : 'Start Tracking Free'}
              </button>
              <button
                onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })}
                className="border border-border text-foreground font-medium px-6 sm:px-8 py-3 sm:py-4 rounded-xl text-sm sm:text-lg hover:bg-muted transition-all w-full sm:w-auto"
              >
                Learn More
              </button>
            </div>
