  ...
  const fetchCurrencySummary = async () => {
    ...
    const currencies = Object.entries(byCurrency)
      .map(([currency, data]) => ({
        ...
        count: data.count,
        total: data.total,
        formatted: formatCurrency(data.total, currency),
      }))
    ...
    return (
      <Card className="glass-card border-outline-variant">
        <CardContent className="p-6 text-center text-destructive">
          Error loading currency summary
        </CardContent>
      </Card>
    )
  }
      ...
      <Card className="glass-card border-outline-variant">
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-on-surface font-headline">Currency Breakdown</CardTitle>
              <p className="text-sm text-on-surface-variant">Current month by currency</p>
            </div>
          </div>
        </CardHeader>
        <CardContent>
      ...
      <CardContent>
        <div className="space-y-3">
          {currencies.map((curr) => (
            <div 
              key={curr.currency}
              className="flex items-center justify-between p-3 bg-surface-container rounded-lg hover:bg-surface-container-high/50 transition-colors"
            >
              <div className="flex items-center gap-3">
