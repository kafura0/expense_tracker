  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-surface-container-high border border-outline-variant rounded-lg p-3 shadow-xl">
        <p className="text-sm text-on-surface-variant mb-1">{d.icon} {d.name}</p>
        <p className="font-bold text-on-surface">${d.value.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
      </div>
    )
  }
  return null
...
export function CategoryChart() {
  ...
  if (orgId === undefined) {
    return (
      <Card className="glass-card border-outline-variant">
        ...
        <CardContent><Skeleton className="h-[300px] w-full rounded-full bg-surface-container-high" /></CardContent>
      </Card>
    )
  }
  ...
  if (error) {
    return <Card className="glass-card border-outline-variant"><CardContent className="p-6 text-center text-destructive">Error loading category data</CardContent></Card>
  }

  if (!data || data.length === 0) {
    return (
      <Card className="glass-card border-outline-variant">
        <CardHeader><CardTitle className="text-on-surface font-headline">Category Breakdown</CardTitle><p className="text-sm text-on-surface-variant">Spending distribution by category</p></CardHeader>
            ...
            {data.map((item, index) => (
              <div key={item.name} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className="font-bold text-on-surface">${item.value.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                  <span className="text-on-surface-variant ml-2 text-sm">({((item.value / total) * 100).toFixed(1)}%)</span>
                </div>
              </div>
            ))}
          </div>
