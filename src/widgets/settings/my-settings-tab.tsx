...
import { getSettings, updateSettings, uploadAvatar, clearPersonalOverride } from '@/features/settings/actions'
...
export function MySettingsTab() {
  ...
  // Remount the form when the server values change so the local state is
  // re-seeded from the latest data (no setState-in-effect needed).
...
function SettingsForm({ settings }: { settings: UserSettingsData }) {
  const [avatarFile, setAvatarFile] = useState<File | null>(null)
  const [avatarPreview, setAvatarPreview] = useState<string | null>(null)
  const [nameError, setNameError] = useState('')
  const [formState, setFormState] = useState<SettingsFormState>({
    displayName: settings.display_name ?? '',
    theme: (settings.theme as SettingsFormState['theme']) || 'dark',
  ...
  const overrideMutation = useMutation({
    mutationFn: (field: 'currency' | 'vat') => clearPersonalOverride(field),
    onSuccess: (res, field) => {
      if (res?.error) {
        toast(res.error, 'error')
        return
      }
      toast('Using the organization default', 'success')
      if (field === 'currency' && settings.org_default_currency) {
        updateField('baseCurrency', settings.org_default_currency)
      }
      if (field === 'vat' && settings.org_default_vat_rate != null) {
        updateField('vatRate', settings.org_default_vat_rate)
      }
      queryClient.invalidateQueries({ queryKey: ['settings'] })
    },
    ...
    <div className="space-y-6">
      ...
      <Card className="glass-card border-border">
        ...
        <CardContent className="space-y-4">
          ...
          {settings.is_org_member && settings.currency_override && (
            <Button
              variant="outline"
              size="sm"
              disabled={overrideMutation.isPending}
              onClick={() => overrideMutation.mutate('currency')}
            >
              <Undo2 className="h-3.5 w-3.5 mr-1.5" />
              Use org default
              {settings.org_default_currency ? ` (${settings.org_default_currency})` : ''}
            </Button>
          )}
          <div className="flex justify-end pt-2 border-t border-border/20">
            <Button
      ...
      <Card className="glass-card border-border">
        ...
        <CardContent className="space-y-4">
          ...
          {settings.is_org_member && settings.vat_override && (
            <Button
              variant="outline"
              size="sm"
              disabled={overrideMutation.isPending}
              onClick={() => overrideMutation.mutate('vat')}
            >
              <Undo2 className="h-3.5 w-3.5 mr-1.5" />
              Use org default
              {settings.org_default_vat_rate != null ? ` (${settings.org_default_vat_rate}%)` : ''}
            </Button>
          )}
          <div className="flex justify-end pt-2 border-t border-border/20">
            <Button
